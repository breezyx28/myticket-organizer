...
## 3. `POST` / `PATCH /api/v1/organizer/events` — accepted fields

**Create (`POST`)** accepts the same seating keys as PATCH (all optional; defaults `layout_type: free`, `rows_count` / `cols_count: 0`).

**Update (`PATCH`)** — validated **snake_case** fields (only send keys that changed):
...
| **`layout_type`** | sometimes, string, **`grid` \| `section` \| `free`** | Seating mode for checkout (default on create: `free`). |
| **`rows_count`** | sometimes, integer, 0–100 | Grid/section map height; forced to **0** when `layout_type` is `free`. |
| **`cols_count`** | sometimes, integer, 0–100 | Grid/section map width; forced to **0** when `layout_type` is `free`. |
...
- **Nested `seats` array** — use **`POST …/events/{id}/seats/bulk`** (§5b).

### 5b. Seating layout — `PATCH` + `POST …/seats/bulk`

**Persist layout on the event (editor save):**

```http
PATCH /api/v1/organizer/events/{id}
{ "layout_type": "grid", "rows_count": 8, "cols_count": 12 }
```

After save, `GET …/events/{id}` returns the same `layout_type`, `rows_count`, `cols_count` (no more silent default to `free` on reload).

**Generate / refresh seat rows** (separate call):

```http
POST /api/v1/organizer/events/{id}/seats/bulk
{
  "layout_type": "grid",
  "rows": 8,
  "cols": 12,
...
| Field | Notes |
|--------|--------|
| `layout_type` | required on bulk; also written to **`events.layout_type`** |
| `rows` / `cols` | optional; map to `events.rows_count` / `events.cols_count` |
| `rows_count` / `cols_count` | optional aliases for the same event columns |
| `replace` | optional bool; when `true`, deletes existing seats unless any are **`booked`** or **`held`** (422 otherwise) |
| `section` | optional label applied to generated seats (section layouts) |

**Response (201):** `{ "created": <int>, "data": { "id", "layout_type", "rows_count", "cols_count" } }`

**Recommended SPA flow:** `PATCH` layout dimensions when the user changes Grid/Section/Free, then `POST …/seats/bulk` with `replace: true` when the map size or layout mode changes (after ticket types exist if assigning `ticket_type_id`).
...
**`POST /api/v1/organizer/events/{id}/cover`**

- **Content-Type:** `multipart/form-data`
- **Field name:** **`image`** (one image, max ~6 MB)
- **Success:** **200** `{ "data": <Event> }`
- Stored on `public` disk under `event-covers/`.
- `data.cover_image_url` is returned as an **absolute** URL.
...
6. `POST …/cover` with multipart **`image`** → `GET` shows `cover_image_url` absolute.
7. `DELETE …/gallery/{id}` → row removed; disk file removed when under `event-gallery/`.
8. `POST …/ticket-types` with `{ "label": "VIP", "default_price": 99 }` → **201** and numeric `id`; `PATCH`/`DELETE` on that id work.
9. `PATCH` with `{ "layout_type": "grid", "rows_count": 4, "cols_count": 6 }` → reload `GET` still shows `grid` / `4` / `6`.
10. `POST …/seats/bulk` with `{ "layout_type": "grid", "rows": 3, "cols": 4, "replace": true }` → `created` > 0 and `data.layout_type` is `grid`.
